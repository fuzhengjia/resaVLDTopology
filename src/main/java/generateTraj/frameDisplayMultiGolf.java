 ...
 * 再显示的时候resize回来。
 *
 * Changes: 改从Spout 读取原始尺寸的frame，将trajectory reshape并画到原始的frame上。
 */
public class frameDisplayMultiGolf extends BaseRichBolt {
    ...
    private float xAdjRatio;
    private float yAdjRatio;

    public frameDisplayMultiGolf(String traceAggBoltNameString) {
    ...
    public void prepare(Map map, TopologyContext topologyContext, OutputCollector outputCollector) {
        ...
        inWidth = ConfigUtil.getInt(map, "inWidth", 320);
        inHeight = ConfigUtil.getInt(map, "inHeight", 240);
        orgWidth = ConfigUtil.getInt(map, "orgWidth", 640);
        orgHeight = ConfigUtil.getInt(map, "orgHeight", 480);

        xAdjRatio = (float)orgWidth / (float)inWidth;
        yAdjRatio = (float)orgHeight / (float)inHeight;
    }
...
//                float point0_y = fscales[ixyScale] * trace.get(0).y();
                float point0_x = fscales[ixyScale] * trace.get(0).x() * xAdjRatio;
                float point0_y = fscales[ixyScale] * trace.get(0).y() * yAdjRatio;
...
//                    float point1_y = fscales[ixyScale] * trace.get(jj).y();
                    float point1_x = fscales[ixyScale] * trace.get(jj).x() * xAdjRatio;
                    float point1_y = fscales[ixyScale] * trace.get(jj).y() * yAdjRatio;
