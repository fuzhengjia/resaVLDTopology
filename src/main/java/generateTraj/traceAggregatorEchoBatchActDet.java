 ...
 */
public class traceAggregatorEchoBatchActDet extends BaseRichBolt {
    ...
    public traceAggregatorEchoBatchActDet(String traceGenBoltNameString, String flowTrackerName) {
...
//                    );
    ...
    public void aggregateTraceRecords(int frameId) {
        ...
        if (traceMonitor.get(frameId) == 0) {
            ...
            List<List<PointDesc>> traceForFeatures = new ArrayList<>();
            for (Map.Entry<String, List<PointDesc>> trace : traceData.entrySet()) {
                int traceLen = trace.getValue().size();
                if (traceLen > maxTrackerLength) {///for getting features!
